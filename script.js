...
// Change task to finished function
...
function renderTask(id, title, description, status) {
    ...
    if (status === 'open') {
        ...
        mainFormDiv.className = 'card-body js-task-open-only';
...
function renderOperation(operationsList, status, operationId, operationDescription, timeSpent) {
    ...
    if (status === 'open') {
        ...
        btn15.innerText = '+15m';
        btn1h.className = 'btn btn-outline-success btn-sm mr-2 js-task-open-only';
        btn1h.innerText = '+1h';
        btnDelete.className = 'btn btn-outline-danger btn-sm js-task-open-only';
